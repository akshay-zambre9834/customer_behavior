...
from customer
group by gender ;
--2]which cusstomer used a discount but still spend more than the avrage purchase amount?
...
WHERE discount_applied = 'Yes'
  ...
  -- 3]which are the top 5 products with the highest avrage rating ?
  select item_purchased,round(avg (review_rating::numeric),2) as "avrage _product_rating"
  from customer 
  group by (item_purchased)
  order by avg (review_rating) desc
  limit 5;
  --4] compare the "avrage purches amount" between "standard" and "express shipping"
  ...
  --5]do subsribed customer send more ? compare avrage spend and total revenue between subscriber and 
  --non subscribers 
...
-- 6]which 5 products are higher purches with discount applied 
select 
        item_purchased,
...
--in quetion that word in used :Segment,Categorize,Classify,Bucket
---->                              	think on case,when suddenly
-- 7]segment customers into new returning and loyal based on their total number of privious purches
select 
case 
...
group by customer_segment;
--........................
--in quetion we found that type of words :
--How many
--Count
--Total customers
--        then COUNT(*).
...
-- 9]ARE customers who are repeat buyers(more than 5 privious purches)also likely to subscribe 
select subscription_status,
